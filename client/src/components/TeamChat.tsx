...
export function TeamChat({ agents, onSendMessage, projectName, gates, artifacts, onResolveGate }: TeamChatProps) {
  ...
  // Get artifacts for the pending gate (artifacts associated with the project)
  const pendingGateArtifacts = pendingGate ? artifacts : [];
                      ...
                      if (msg.role === 'tool' && msg.toolName) {
                        ...
                        return (
                          <div key={msg.id} className="animate-fade-in w-full">
                            ...
                            >
                              <div
                                className={`
                                  ...
                                  border-l-2 ${isToolRunning ? 'border-l-orange' : 'border-l-green'}
                                ...
                                <div className="flex items-start gap-3 py-2.5 px-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="font-mono text-xs text-text-muted truncate">
                                      {tool.desc || (msg.toolInput ? JSON.stringify(msg.toolInput).slice(0, 60) : '')}
                                    </div>
                                  </div>
                            ...
                            {isExpanded && (
                              <div className="bg-bg-secondary border-l-2 border-l-border rounded-b-lg px-3 pb-3 pt-1 space-y-2 ml-0">
                                {msg.toolInput && Object.keys(msg.toolInput).length > 0 && (
                                  <div>
                                    ...
                                    <pre className="font-mono text-xs overflow-x-auto text-text-secondary bg-bg-primary/50 rounded p-2 max-h-[200px] overflow-y-auto">
                ...
                <div className="flex flex-col gap-1.5 flex-1 max-w-[85%]">
                  ...
                  <div className="bg-bg-elevated border border-orange/30 rounded-2xl rounded-tl-md p-4">
                    ...
                    <div className="mt-3 pt-3 border-t border-border/50 text-xs text-text-muted">
                      Requested by <span className="text-orange font-semibold">{pendingGate.resolvedBy || 'Agent'}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {pendingGate ? (
        <div className="flex-shrink-0 border-t border-orange/30 bg-bg-secondary/80 backdrop-blur-sm p-3">
          <div className="max-w-3xl mx-auto flex items-center gap-3">
            ...
            <button
              onClick={() => {
                onResolveGate(pendingGate.id, 'approved', gateComment || undefined);
                setGateComment('');
              }}
              className="
            ...
            <button
              onClick={() => {
                onResolveGate(pendingGate.id, 'changes_requested', gateComment);
                setGateComment('');
              }}
